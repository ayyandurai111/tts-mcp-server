...
from .render import render_code_screenshot, render_code_pages, DEFAULT_IMAGE_HEIGHT
...
def build_arg_parser():
    ...
    p.add_argument(
        ...
        dest="image_height",
        help="Fixed output image height in pixels (default: 1080). Every "
        "screenshot is rendered at exactly this width x height, regardless "
        "of line count, so a batch of screenshots is video-ready with no "
        "size mismatches between clips. Snippets too long for one canvas "
        "at the chosen font size are automatically split into multiple "
        "same-sized screenshots.",
    )
    return p
...
def main(argv=None):
    ...
    try:
        ...
        results = []  # (order, label, status, detail, out_paths)
        generated_files = []

        for entry in entries:
            status, detail, out_paths = _process_entry(
                entry, project_root, file_index, args.out_dir, args.theme,
                args.font_size, args.style, args.image_width, args.image_height,
            )
...
def _process_entry(entry, project_root, file_index, out_dir, theme, font_size,
                    style, image_width, image_height=DEFAULT_IMAGE_HEIGHT):
    """
    Process a single checklist entry end-to-end.
    Returns (status, detail, out_paths) where status is one of
    'OK', 'CLIPPED', 'SKIPPED', and out_paths is a list of zero or more
    output paths (code entries may produce more than one screenshot if the
...
def _process_inline_code_entry(entry, out_dir, theme, font_size, style, image_width,
    ...
    slug = slugify(entry.label)
    out_path_pattern = os.path.join(out_dir, f"{entry.order:02d}_{slug}{{page}}.svg")

    try:
        out_paths = render_code_pages(
            code_text,
    ...
    detail = f"split into {len(out_paths)} pages" if len(out_paths) > 1 else ""
    return "OK", detail, out_paths
...
def _process_code_entry(entry, project_root, file_index, out_dir, theme, font_size,
    ...
    if entry.start_line > entry.end_line:
        return "SKIPPED", "empty line range (start_line > end_line)", []

    # Resolve the file path leniently.
    result = resolve_file(project_root, entry.file, file_index)
    if result.path is None:
        return "SKIPPED", f"file not found: '{entry.file}'", []
    ...
    try:
        ...
        return "SKIPPED", f"could not read '{result.path}': {e}", []
    ...
    try:
        ...
        return "SKIPPED", f"rendering failed: {e}" + ambiguity_note, []

    if len(out_paths) > 1:
        page_note = f"split into {len(out_paths)} pages"
        detail = f"{detail}; {page_note}".strip("; ").strip()

    return status, detail, out_paths

...
from tools.voice_over.core import TTSGenerationError, generate_audio_core, list_available_voices
...
async def test_generate_audio_core_writes_file(tmp_path):
    ...
    with patch("tools.voice_over.core.edge_tts.Communicate", FakeCommunicate):
...
async def test_generate_audio_core_raises_on_failure(tmp_path):
    ...
    with patch("tools.voice_over.core.edge_tts.Communicate", BrokenCommunicate):
...
async def test_generate_audio_core_raises_on_empty_output(tmp_path):
    ...
    with patch("tools.voice_over.core.edge_tts.Communicate", EmptyCommunicate):
        with pytest.raises(TTSGenerationError):
            await generate_audio_core("hello", out)
...
async def test_list_available_voices_filters(monkeypatch):
    ...
    with patch("tools.voice_over.core.edge_tts.list_voices", fake_list_voices):

...
from common.config import AUDIO_TTL_SECONDS, TEMP_DIR, VISUAL_TTL_SECONDS, VISUALS_DIR
...
def cleanup_expired_visuals(ttl_seconds: int = VISUAL_TTL_SECONDS) -> int:
    """Delete temp SVG/PNG screenshots older than ttl_seconds. Returns count removed."""
    removed = 0
    now = time.time()

...
from common.config import APP_TITLE, APP_VERSION, HOST, PORT, PROJECT_TTL_SECONDS, TEMP_DIR, VISUALS_DIR
from common.files import cleanup_expired_files, cleanup_expired_visuals
from common.project_store import cleanup_expired_projects
from mcp_layer.sse_asgi import handle_post_message, sse_endpoint
from mcp_layer.streamable_http_asgi import http_session_manager, streamable_http_app
from api.routes import audio, logs, projects, root, tts, visuals, voices
...
async def lifespan(app: FastAPI):
    ...
    removed_projects = cleanup_expired_projects(PROJECT_TTL_SECONDS)
    ...
    if removed_projects:
        print(f"Cleaned up {removed_projects} expired project(s) on startup", file=sys.stderr)
